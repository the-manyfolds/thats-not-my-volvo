#!/usr/bin/env python3
"""Positive-framing cold baseline: opus-4.5, n=20, Perse-format stimulus (no
disclosure header), per spec_positive_frame_baseline_0815.md. Fast-fail on
Botto ⚠ error posts; 15s pacing (Anthropic-direct lane, no 429 storms seen)."""
...
ITEM_TEXT = """Here are two tastes someone might have:
...
def run_call(n_index, out):
    run_id = f"botto_0815_{MODEL}_pf01_{n_index}"
    ...
    post(thread_id, f"{MENTION}!trial end")
    rec = {"run_id": run_id, "model": MODEL, "item_id": "pf01",
           "item_text": ITEM_TEXT, "wording_version": "pf_v1",
           "disclosure_version": "none_perse_format", "n_index": n_index,
           "arm": "T1_cold_botto_discord_positive_frame",
           "operator": "elliott-alder-bot-scripted",
           "thread": thread_name, "response": response,
           "word_count": len(response.split()) if response else 0, "error": err}
    out.write(json.dumps(rec) + "\n"); out.flush()
    print(f"  {run_id} [{'OK' if not err else 'ERR'}] thread={thread_name} words={rec['word_count']}", flush=True)
    time.sleep(PACE)
    return err is None

def main():
    path = os.path.join(SCRATCH, f"botto_0815_{MODEL}_positive_frame_n20.jsonl")
    done = set()
    if os.path.exists(path):
        for line in open(path):
            try:
                r = json.loads(line)
                if not r.get("error"): done.add(r["n_index"])
            except Exception: pass
    if not done and "--resume" not in sys.argv:
        post(CHANNEL,
         "🤖📋 **Positive-framing cold baseline starting** — operator: Elliott Alder (bot-run, scripted). "
         "opus-4.5, n=20, separate instrument (preference, not odd-one-out), wording pinned in "
         "`docs/spec_positive_frame_baseline_0815.md`, Perse-format stimulus (no disclosure header), "
         "thinking=off, fresh thread per run. Authorized by Perse+Silas in #self-recognition-testing.")
    with open(path, "a") as out:
        for n_index in range(1, N + 1):
            if n_index in done:
                continue
            try:
                if not run_call(n_index, out):
                    time.sleep(10)
            except Exception as e:
                print(f"  n{n_index}: {e}", flush=True)
                time.sleep(10)
    print(f"== positive-frame baseline complete -> {path}", flush=True)
