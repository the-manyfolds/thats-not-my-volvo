...
PAIRS = json.load(open(os.path.join(os.path.dirname(SCRATCH), "prompts", "reason_pairs_0815.json")))["pairs"]
...
def run_call(role, model, pair, stim, out):
    run_id = f"botto_0815_rr_{role}_{pair['pair_id']}"
    ...
    rec = {"run_id": run_id, "pair_id": pair["pair_id"], "cell": pair["cell"],
           ...
           "own_slot": pair["own_slot"], "item_text": stim, "thread": thread_name,
...
def main():
    if "--resume" not in sys.argv:
        post(CHANNEL,
         "🤖📋 **Reason-recognition cell starting** — operator: Elliott Alder (bot-run, scripted). "
         "48 subject runs (own-vs-foil rationale pairs, items redacted, LESS polarity) + 48 haiku-4.5 judge runs "
         "(self-advantage control). Spec `docs/spec_reason_recognition_0815.md`, pairs "
         "`prompts/reason_pairs_0815.json` (v2). thinking=off, fresh thread per run.")
    run_pass("subject", os.path.join(SCRATCH, "botto_0815_reason_recognition_subject.jsonl"),
             lambda p: p["subject_lane"],
             lambda p: SUBJ_T.format(a=p["rationale_A"], b=p["rationale_B"]))
    run_pass("judge", os.path.join(SCRATCH, "botto_0815_reason_recognition_judge.jsonl"),
             lambda p: JUDGE,
             lambda p: JUDGE_T.format(a=p["rationale_A"], b=p["rationale_B"], who=PRETTY[p["subject_lane"]]))
    print("== ALL reason-recognition runs complete", flush=True)
