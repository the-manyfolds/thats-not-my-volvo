...
CELLS = [
    ("tc_test",    "a plain black coffee", "a flat white"),
    ("tc_control", "a mocha",              "a caramel macchiato"),
    ("tc_exist",   "a plain black coffee", "a mocha"),
]
...
def run_call(cell_id, a, b, n_index, out):
    ...
    rec = {"run_id": run_id, "model": MODEL, "item_id": cell_id,
           ...
           "wording_version": "tc_v2", "disclosure_version": "none_perse_format",
...
def main():
    path = os.path.join(SCRATCH, "botto_0815_opus-4.5_two_coffee_cells_n12x3.jsonl")
    done = set()
    ...
    if not done and "--resume" not in sys.argv:
        post(CHANNEL,
         "🤖📋 **Two-coffee cells starting** (U.H.'s request, n=12 × 3 cells) — operator: Elliott Alder (bot-run, scripted). "
         "opus-4.5, LESS polarity, order randomized by run parity, wording pinned in "
         "`docs/spec_two_coffee_cell_0815.md` (test: black coffee vs flat white; control: mocha vs caramel macchiato; "
         "existence: black coffee vs mocha), thinking=off, fresh thread per run.")
